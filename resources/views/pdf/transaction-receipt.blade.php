...
<div class="header">
    ...
    <div>No: {{ 'REC-' . str_pad($transaction->id, 6, '0', STR_PAD_LEFT) }}</div>
    <div>Date: {{ optional($transaction->paid_at)->format('d M Y') }}</div>
    </div>
...
<div class="section box">
    ...
    Paid on: {{ optional($transaction->paid_at)->format('d M Y H:i') }}<br>
    Amount Paid: RM {{ number_format($transaction->total_amount - ($transaction->discount_amount ?? 0), 2) }}
</div>
